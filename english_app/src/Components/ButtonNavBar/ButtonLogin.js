...
import { UserLogin } from "../../Creators/UserCreator";
import AlertErrorInput from "../Notification/AlertErrorInput";

export default function ButtonLogin() {
  const [visible, setVisible] = React.useState(true);
  const IsShow = useSelector((state) => state.showLogin.show);
  ...
  const handleLogin = async () => {
    ...
    try {
      let info = await Apis.get(endpoints["outh2Info"]);
      if (info !== null) {
        let res = await Apis.postForm(endpoints["login"], {
          grant_type: "password",
          username: username,
          password: password,
        ...
        if (res !== null) {
          let user_data = await Apis.get(endpoints["currentUser"], {
            headers: {
              Authorization: `Bearer ${res.data.access_token}`,
            },
          });
          if (user_data !== null) {
            let profile = await Apis.get(
              endpoints["getProfile"](user_data.data.id),
              {
                headers: {
                  Authorization: `Bearer ${res.data.access_token}`,
                },
            ...
            if (profile !== null) {
              ...
              cookies.save("current-user", user_data.data);
              cookies.save("profile", profile.data.profile);
              dispatch(UserLogin([user_data.data, profile.data]));
              dispatch(HiddenLogin());
            } else {
  ...
  return (
    <div className="SiteNavLoginSection-loginButton RightNavigationItem">
      <button
        className="AssemblyButtonBase AssemblyTextButton AssemblyTextButton--secondary AssemblyButtonBase--medium AssemblyButtonBase--padding"
          ...
          <section className="cvsxyiq">
            ...
            <div className="a1731mpn">
              <div className="axxq1d5">
                ...
                <div className="b1r6bd1m">
                  <div className="bq40tjb">
                    <div className="hideAbove--s">
                      <a className="AssemblyButtonBase AssemblySecondaryButton AssemblyButtonBase--large AssemblyButtonBase--padding AssemblyButtonBase--fullWidth">
                ...
                <form className="LoginPromptModal-form ">
                  <div className="fjm13bn" style={{ "--fjm13bn-0": "1.5rem;" }}>
                    {errors.length > 0 ? (
                      <AlertErrorInput errors={errors} />
                    ) : (
                      ""
                    )}
                    <label className="UIInput" htmlFor="username">
                      ...
                      <span className="UIInput-label">Username</span>
                    </label>
                    <label className="UIInput">
                      ...
                      <div className="UIInput-labelRow">
                        <div className="UIInput-labelRowLeft notranslate">
                          PASSWORD
                        </div>
                        <div className="UIInput-labelRowRight">
                          <div className="LoginPromptModal-forgotPasswordLinkWrapper">
                            <a
                              className="UILink"
                  ...
                  <br />
                  <button
                    className="UIButton UIButton--fill UIButton--hero"
                    type="button"
                    onClick={handleLogin}
                    id="btn-login"
                  >
                    <span className="UIButton-wrapper">
                      <div className="UILoadingButton">
                        {isLoad ? (
                          <div
                            className="UILoadingIndicator UILoadingIndicator--medium"
                            data-testid="UILoadingIndicator"
                          >
                            <svg className="UILoadingIndicator-svg">
                              <circle
                                className="UILoadingIndicator-path"
                                cx="11"
                                cy="11"
                                fill="none"
                                r="8.8"
                ...
                <br />
                <div className="LoginPromptModal-signUpCTAContainer">
                  <div className="UIDiv LoginPromptModal-signUpCTA">
                    <span className="LoginPromptModal-signUpCTAMessage">
                      New to Quizlet?{" "}
                      <button
                        className="UILink"
                        type="submit"
